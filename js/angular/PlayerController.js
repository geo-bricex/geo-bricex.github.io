...
app.controller('controllerPlayer', function ($scope, $http) {// controller del inicio

    let paramUrl = "fun", VoR = "true";
    let participantes = [];
    var elected = "";
    ...
    $scope.playgame = () => {
        if (document.getElementsByTagName("td").length > 3) {
            ...
            Swal.fire({
                icon: 'error',
                title: 'Oops...',
                text: 'To start the game there must be more than two participants',
                footer: '<a href="https://www.facebook.com/geovannybricex">Contact me if you do not have friends :)</a>'
            });
        }
    };
    ...
    apiStartGame = (api_param) => {
        $.ajax({
            ...
            success: (data) => {
                ...
                } while (victimChallenge === elected || victimChallenge === "All");
